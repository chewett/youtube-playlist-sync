...
def sync_playlist(config_path):
    ...
    if config_data['format'] != None:
        if shutil.which('ffmpeg') == None:
            ...
            run_in_subprocess([YOUTUBE_DL_PATH, '--extract-audio', '--audio-format', config_data['format'], '-o', config_data['directory'] + r'\%(title)s.%(ext)s', config_data['youtube_playlist']])
    else:
        run_in_subprocess([YOUTUBE_DL_PATH, config_data['youtube_playlist'], '-o', config_data['directory'] + r'\%(title)s.%(ext)s'])
